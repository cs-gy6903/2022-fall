...
def problem3(g: int, p: int) -> DHKeyPair:
    ...
    {'a': 12, 'A': 13}
    """
    a = g + 1 + secrets.randbelow(p - g - 1)
    return {"a": a, "A": problem2(g, a, p)}
...
def problem4(g: int, p: int, a: int, A: int) -> bool:
    ...
    True
    """
    return a > 1 and g < a and a < p and A < p and A == problem2(g, a, p)
...
def problem5(g: int, p: int, B: int, b: int = None) -> DHNegotiatedSecret:
    ...
    {'A': 10, 's': 2}
    """
    keypair: DHKeyPair = problem3(g, p)
    a, A = keypair["a"], keypair["A"]
    s = problem2(B, a, p)
    return {"s": s, "A": A}
